...
def _paths(runtime, doc, end):
    ...
    paths = (
        sorted(
            ...
            if retention.FILENAME.fullmatch(p.name) and p.stem <= last
        )
        if directory.exists()
        else []
    )
...
def _archive(payload, path):
    ...
    if (
        ...
        or archive.get("schema") != 1
        or archive.get("day") != path.stem
    ):
...
def _ranges(events, windows, end):
    ...
    for begin, stop in windows:
        for arm in ARMS:
            ...
            if arm == "baseline":
                ranges.extend(
                    (t["opened_at"], t["opened_at"]) for t in selected if t["opened_at"] is not None
                )
    return ranges


def _needed(path, ranges):
    return any(retention._day(begin) <= path.stem <= retention._day(end) for begin, end in ranges)
...
def _boundary_days(indexes, windows):
    ...
    for start, _ in windows:
        for arm in ARMS:
            prior = [
                (row["equity_times"][arm], path)
                for path, row in indexes.items()
...
def _select_observations(rows, ranges, windows):
    selected = {
        row["time"] for row in rows if any(begin <= row["time"] <= stop for begin, stop in ranges)
    }
    for start, _ in windows:
        for arm in ARMS:
            prior = [
                ...
                for row in rows
                if row["time"] <= start
                and row.get("telemetry_schema") == 1
...
def _observations(paths, indexes, retained, doc, ranges, windows, end):
    ...
    merged = {
        kind: retention._merge([], [r for r in records if r["time"] <= end], kind)
        for kind, records in rows.items()
    }
    merged["observations"] = _select_observations(merged["observations"], ranges, windows)
    merged["errors"] = [
        r for r in merged["errors"] if any(begin <= r["time"] <= stop for begin, stop in windows)
    ]
    return merged
