...
def _quote(symbol,price,at,add):
    return dict(symbol=symbol,bid=price,ask=price*1.0001,mark=price,
        bid_size=1000000,ask_size=1000000,quote_time=at,lot_size=1,
        multiplier=.001,tick_size=.001,funding_rate=.001,
        funding_interval_hours=8,score=70,eligible=True,add_eligible=add,
        atr_pct=2,entry_edge_pct=4,turnover24h=10000000)


def _features(at):
    return dict(fetched_at=at,symbols={symbol:dict(eligible=index==1,
...
def document():
    ...
    doc=dict(schema=1,mode='paper',start_at=SAMPLE_START,config=config,
        tick_seconds=SAMPLE_TICK,report_seconds=SAMPLE_TICK,events=[],observations=[],
        history=[],errors=[],accounts={})
    for index,prices in enumerate(SAMPLE_PRICES):
        at=SAMPLE_START+(index+1)*SAMPLE_TICK
        market={symbol:_quote(symbol,price,at,index==1)
                for symbol,price in zip(SAMPLE_SYMBOLS,prices)}
        features=_features(at); equity={}
        for arm in ARMS:
            inputs=coinglass.apply_filter(market,features,at) if arm=='liquidation_filter' else market
        ...
        doc['observations'].append(dict(time=at,market=market,coinglass=features,
            scan=dict(scanned_at=at,top5=list(SAMPLE_SYMBOLS),liquid_contracts=2),
            telemetry_schema=1,equity=equity))
    doc['last_tick_at']=at
    doc['accounts']={arm:dict(state=states[arm],events=[],statistics=dict(initial_equity=1000))
                     for arm in ARMS}
    return doc
...
def report():
    ...
    result['summary']='SYNTHETIC FIXTURE ONLY. Generated quotes and paper fills; no market data or profitability claim.'
    return result
...
def write_sample(output_directory):
    ...
    result=report();page,_=audits._render(result)
    contents={'json':json.dumps(result,indent=2,allow_nan=False)+'\n','html':page}
    targets={extension:directory/(SAMPLE_OUTPUT_STEM+'.'+extension) for extension in contents}
    if any(path.is_symlink() or (path.exists() and not path.is_file()) for path in targets.values()):
