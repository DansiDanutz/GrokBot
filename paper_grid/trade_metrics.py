...
from paper_grid.metric_constants import (
    ARMS,
    CLOSE_REASONS,
    SYMBOL,
    SECONDS_PER_HOUR,
...
def _closed(event, position, symbol, strict):
    ...
    return dict(
        ...
        symbol=symbol,
        adds=position.get("adds"),
        entry_notional=position.get("entry_notional"),
        net_pnl=pnl,
        entry_fees=cost("entry_fees"),
...
def lifecycles(events, end, strict=False):
    ...
    for event in sorted(events, key=lambda e: e["time"]):
        ...
        if event["time"] > end or arm not in ARMS or kind not in ("open", "add", "close"):
            continue
        symbol = _symbol(event.get("symbol"), strict)
        key = (arm, symbol)
        if kind == "open":
            if key in active:
            ...
            active[key] = dict(
                opened_at=event["time"], adds=0, entry_notional=notional, _fills=[event]
            )
        elif kind == "add":
            if strict:
            ...
            if key in active:
                old = active[key]
                total = old.get("entry_notional")
                extra = event.get("notional")
                active[key] = dict(
                    old,
                    adds=old["adds"] + 1 if old["adds"] is not None else None,
                    entry_notional=total + extra
                    if total is not None and extra is not None
                    else None,
...
def closed_trades(events, end):
    ...
    return {
        arm: [{k: v for k, v in t.items() if not k.startswith("_")} for t in rows]
...
def _exposure(doc, arm, start, end, closed, active):
    ...
    for symbol in set(current) - known_symbols:
        ...
        if not engine._number(opened) or opened < end:
            unknown += 1
    seconds = _union(intervals)
    duration = end - start
    return dict(
        observed_seconds=seconds,
        fraction=seconds / duration if duration and not unknown else None,
...
def _filter_state(trade, observations):
    ...
    if obs is None or trade["opened_at"] is None or not isinstance(obs.get("coinglass"), dict):
        return "unknown"
    symbol = trade["symbol"]
    quote = obs.get("market", {}).get(symbol)
...
def _cohort(trades, observations):
    ...
    return dict(
        ...
        allowed_trades=len(trades) - len(blocked) - unknown,
        unknown_trades=unknown,
        net_pnl=sum(t["net_pnl"] for t in blocked),
        coverage="partial" if unknown else "complete_recorded_entries",
        interpretation=COHORT_NOTE,
    )
...
def _fill(position, event, record, at, c, symbol):
    ...
    if not engine._number(event.get("fee")):
        return None
    old = position or dict(
        quantity=0, cost_basis=0, entry_fees=0, contracts=0, funding_accrued=0, funding_time=at
    )
    return dict(
        old,
        quantity=old["quantity"] + event["notional"] / event["fill_price"],
        cost_basis=old["cost_basis"] + event["notional"],
        entry_fees=old["entry_fees"] + event["fee"],
        contracts=old["contracts"] + event["contracts"],
        last_mark=record["mark"],
        last_bid=record["bid"],
        funding_rate=max(record["funding_rate"], 0),
        funding_interval_hours=record["funding_interval_hours"],
    )
...
def _excursion_result(values, missing, available):
    return dict(
        observed_mae_net_usdt=min(values) if available and values else None,
        observed_mfe_net_usdt=max(values) if available and values else None,
        excursion_samples=len(values) if available else 0,
        excursion_missing_samples=missing,
        excursion_coverage=("partial" if missing else "complete_observations")
        if available
        else "unavailable",
    )
...
def _excursions(trade, observations, config, tick_seconds):
    ...
    for at in sorted(set(rows) | set(fills)):
        obs = rows.get(at)
        record = obs.get("market", {}).get(trade["symbol"]) if obs else None
        gap = max(0, math.floor((at - previous) / tick_seconds) - 1)
        missing += gap
        previous = at
...
def _summary(trades):
    ...
    for trade in trades:
        symbol = trade["symbol"]
        old = symbols.get(symbol, dict(symbol=symbol, closes=0, net_pnl=0))
        symbols[symbol] = dict(
            old, closes=old["closes"] + 1, net_pnl=old["net_pnl"] + trade["net_pnl"]
        )
    return dict(
        ...
        known_hold_trades=len(holds),
        unknown_hold_trades=len(trades) - len(holds),
...
def report(doc, arm, start, end, include_start=False, include_end=True):
    ...
    selected = [
        t
        for t in closed[arm]
        if calendar_day.contains(
            t["closed_at"], start, end, include_start=include_start, include_end=include_end
        )
    ]
    ...
    decorated = [
        dict(
            ...
            **_excursions(
                t, observations, doc.get("config"), doc.get("tick_seconds", DEFAULT_TICK_SECONDS)
            ),
        )
    ...
    result.update(
        exposure=_exposure(doc, arm, start, end, closed[arm], active),
        trades=decorated,
        trades_total=len(selected),
        excursion_definition=EXCURSION_NOTE,
