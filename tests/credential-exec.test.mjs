...
test('private loader allows the fixed TypeSafe key and rejects arbitrary environment keys',t=>{const {file}=fixture(t);writeFileSync(file,JSON.stringify({DLS_TELEGRAM_BOT_TOKEN:'synthetic-value',TYPESAFE_API_KEY:'synthetic-typesafe'}),{mode:0o600});let r=spawnSync('python3',[loader,'--credentials',file,'--','/usr/bin/env','python3','-c','import os,sys; sys.exit(0 if os.environ.get("TYPESAFE_API_KEY")=="synthetic-typesafe" else 2)'],{encoding:'utf8'});assert.equal(r.status,0);writeFileSync(file,JSON.stringify({PATH:'/tmp'}),{mode:0o600});r=run(file);assert.equal(r.status,1)});
